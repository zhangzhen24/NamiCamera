...
UNamiCameraEffectComponent::UNamiCameraEffectComponent()
...
void UNamiCameraEffectComponent::Activate_Implementation()
{
...
void UNamiCameraEffectComponent::Deactivate_Implementation()
{
...
void UNamiCameraEffectComponent::Update_Implementation(float DeltaTime)
{
...
void UNamiCameraEffectComponent::ApplyToView_Implementation(FNamiCameraView& InOutView, float DeltaTime)
{
...
void UNamiCameraEffectComponent::ActivateEffect(bool bResetTimer)
{
	...
	NAMI_LOG_EFFECT(Log, TEXT("[UNamiCameraEffectComponent] 激活效果：%s，持续时间：%.2f"),
		*EffectName.ToString(), Duration);
}

void UNamiCameraEffectComponent::DeactivateEffect(bool bForceImmediate)
{
	if (!bIsActive)
	{
		return;
	...
	else
	{
		...
		NAMI_LOG_EFFECT(Warning, TEXT("[UNamiCameraEffectComponent] 开始混出：%s，起始权重=%.3f"),
			*EffectName.ToString(), BlendOutStartWeight);
	}

	NAMI_LOG_EFFECT(Log, TEXT("[UNamiCameraEffectComponent] 停止效果：%s，立即：%s"),
		*EffectName.ToString(), bForceImmediate ? TEXT("是") : TEXT("否"));
}

void UNamiCameraEffectComponent::PauseEffect()
{
	bIsPaused = true;
	NAMI_LOG_EFFECT(Verbose, TEXT("[UNamiCameraEffectComponent] 暂停效果：%s"), *EffectName.ToString());
}

void UNamiCameraEffectComponent::ResumeEffect()
{
	bIsPaused = false;
	NAMI_LOG_EFFECT(Verbose, TEXT("[UNamiCameraEffectComponent] 恢复效果：%s"), *EffectName.ToString());
}


float UNamiCameraEffectComponent::CalculateBlendWeight(float DeltaTime)
{
	float Weight = 1.0f;
...
void UNamiCameraEffectComponent::ApplyEffect_Implementation(FNamiCameraView& InOutView, float Weight, float DeltaTime)
{
	// 基类实现为空，子类应该重写
}
