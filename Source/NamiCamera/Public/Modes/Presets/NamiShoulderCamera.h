...
protected:
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Shoulder Camera|Offset",
              meta = (Tooltip = "相机相对于角色中心的水平偏移，正数表示右侧，负数表示左侧"))
    ...
    /** 瞄准模式相机距离 */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Shoulder Camera|Aiming",
              meta = (EditCondition = "bEnableAimingMode", ClampMin = "50.0", ClampMax = "500.0",
                      Tooltip = "进入瞄准模式时的相机距离"))
    float AimingCameraDistance = 150.0f;

    /** 瞄准模式FOV */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Shoulder Camera|Aiming",
              meta = (EditCondition = "bEnableAimingMode", ClampMin = "30.0", ClampMax = "90.0",
                      Tooltip = "进入瞄准模式时的视野角度，通常比默认FOV小，提供更精确的瞄准体验"))
    float AimingFOV = 60.0f;

    /** 瞄准模式平滑时间 */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Shoulder Camera|Aiming",
              meta = (EditCondition = "bEnableAimingMode", ClampMin = "0.0", ClampMax = "0.5",
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Shoulder Camera|Behavior",
              meta = (Tooltip = "相机是否基于角色朝向进行定位，建议开启以获得更好的跟随效果"))
    bool bUseCharacterFacing = true;

    /** 角色朝向影响强度 */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Shoulder Camera|Behavior",
              meta = (EditCondition = "bUseCharacterFacing", ClampMin = "0.0", ClampMax = "1.0",
