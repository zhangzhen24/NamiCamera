...
/**
 * 相机调整动画通知状态
 * 用于在动画中控制相机参数调整和效果，支持修改所有相机参数、相机震动、输入打断、BlendIn/BlendOut混合。
 * 自动使用动画总时长作为 Duration。
 */
UCLASS(meta = (Tooltip = "相机调整动画通知。用于在动画中控制相机参数调整和效果，支持修改所有相机参数、相机震动、输入打断、BlendIn/BlendOut混合。"))
...
public:
	...
	/** 枢轴点位置修改。枢轴点是相机围绕旋转的中心点，通常是角色位置。Additive模式在当前位置基础上偏移，Override模式直接设置目标位置。 */
	...
	/** 枢轴点旋转修改。基础旋转，决定相机的基础朝向。Additive模式在当前旋转基础上叠加，Override模式直接设置目标旋转。 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "1. 枢轴点",
	...
	/** 吊臂长度调整。调整相机与角色之间的距离。Additive模式正值拉远负值拉近，Override模式直接设置目标距离。 */
	...
	/** 吊臂旋转调整。调整吊臂方向，以角色位置为中心，以吊臂长度为半径旋转。主要影响相机位置，间接影响相机朝向。可通过控制模式设置是否允许玩家输入打断。 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "2. 吊臂",
			  meta = (Tooltip = "调整吊臂方向，以角色位置为中心，以吊臂长度为半径旋转。\n主要影响相机位置（让相机沿以角色为中心的圆弧移动），间接影响相机朝向。\nPitch：俯仰（正值向上，形成俯视角）。\nYaw：偏航（正值向右绕行）。\n可通过控制模式设置是否允许玩家输入打断。"))
	FNamiRotatorModify ArmRotation;

	/** 吊臂偏移。在吊臂本地空间中的偏移，常用于肩部视角切换（左右偏移）。Additive模式在当前偏移基础上叠加，Override模式直接设置目标偏移。 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "2. 吊臂",
	...
	/** 视角控制模式。建议视角：玩家输入可打断（推荐用于技能演出）。强制视角：忽略玩家输入（用于过场动画、QTE）。混合视角：玩家输入衰减效果。仅在允许玩家输入关闭时生效。 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "3. 视角控制",
			  meta = (EditCondition = "ArmRotation.bEnabled && !bAllowPlayerInput",
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "3. 视角控制",
			  meta = (EditCondition = "ArmRotation.bEnabled && !bAllowPlayerInput && ControlMode != ENamiCameraControlMode::Forced",
	...
	/** 相机位置偏移（相机本地空间）。在吊臂计算完成后，额外的相机位置偏移，用于微调相机位置，如技能特效时的相机抖动。Additive模式在当前偏移基础上叠加，Override模式直接设置目标偏移。 */
	...
	/** 相机旋转偏移。只改变相机朝向，不影响相机位置。用于看向特定方向或目标。不受玩家输入影响。 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "4. 相机",
			  meta = (Tooltip = "只改变相机朝向，不影响相机位置。\n用于看向特定方向或目标。\nPitch：俯仰（正值向上看）。\nYaw：偏航（正值向右看）。\nRoll：翻滚。\n不受玩家输入影响。"))
	FNamiRotatorModify CameraRotationOffset;

	/** FOV调整。调整相机的视野角度（Field of View）。Additive模式正值广角负值长焦，Override模式直接设置目标FOV。 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "4. 相机",
	...
	/** 相机震动。选择继承自UCameraShakeBase的震动类，用于在动画期间产生相机震动效果。震动效果会在动画开始时激活，在动画结束时停止（根据结束行为）。 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "6. 效果",
			  meta = (Tooltip = "相机震动效果类。\n选择继承自 UCameraShakeBase 的震动类，用于在动画期间产生相机震动效果。\n震动效果会在动画开始时激活，在动画结束时停止（根据结束行为）。"))
	TSubclassOf<UCameraShakeBase> CameraShake;

	/** 震动强度。震动效果的强度倍数。1.0为原始强度，大于1.0增强震动，小于1.0减弱震动。建议范围0.0-5.0。 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "6. 效果",
			  meta = (EditCondition = "CameraShake != nullptr",
					  ClampMin = "0.0", ClampMax = "5.0",
					  Tooltip = "震动效果的强度倍数。\n1.0 为原始强度，大于 1.0 增强震动，小于 1.0 减弱震动。\n建议范围：0.0 - 5.0。"))
	float ShakeScale = 1.0f;

	/** 激活前是否强制停止已有技能镜头。开启后，在创建本次镜头前会停止当前玩家上的所有相机效果，避免技能镜头叠加。 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "6. 效果",
			  meta = (Tooltip = "开启后，在创建本次镜头前会停止当前玩家上的所有相机效果，避免技能镜头叠加。"))
	...
	/** 混出时保持相机旋转。如果启用，在混出时相机旋转会保持在混出开始时的状态，不会混合回默认值。如果禁用，相机旋转会正常混合回默认值。默认启用，适用于大多数技能镜头场景。 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "7. 混合",
			  meta = (Tooltip = "如果启用，在混出（BlendOut）时，相机旋转会保持在混出开始时的状态，不会混合回默认值。\n如果禁用，相机旋转会正常混合回默认值。\n默认启用，适用于大多数技能镜头场景。"))
	bool bPreserveCameraRotationOnExit = true;

	/** 最大角度变化速度（度/秒）。用于限制PivotRotation的变化速度，防止鼠标快速输入时导致PivotLocation跳变。0表示不限制（使用最短路径直接变化）。建议值：默认720.0（每秒2圈），快速响应1080.0（每秒3圈），平滑响应360.0（每秒1圈）。 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "7. 混合",
	...
	/** 手动清理Tag。用于标记需要手动清理的Feature。当EndBehavior为Stay时，会自动添加此Tag到Feature。留空则使用默认值Camera.Feature.ManualCleanup。 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "GameplayTags",
			  meta = (Tooltip = "用于标记需要手动清理的 Feature。留空则使用默认值 Camera.Feature.ManualCleanup。"))
	FGameplayTag StayTag;

	/** 来源标识Tag。用于标识此Feature的来源。当EndBehavior为Stay时，会自动添加此Tag到Feature。留空则使用默认值Camera.Feature.Source.ANS.CameraAdjust。 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "GameplayTags",
			  meta = (Tooltip = "用于标识此 Feature 的来源。留空则使用默认值 Camera.Feature.Source.ANS.CameraAdjust。"))
