...
/**
 * 相机效果组件基类
 * 提供生命周期管理和混合控制，用于需要 BlendIn/BlendOut、Duration、输入打断等功能的效果组件。
 */
UCLASS(Abstract, Blueprintable, BlueprintType)
class NAMICAMERA_API UNamiCameraEffectComponent : public UNamiCameraModeComponent
{
	GENERATED_BODY()

public:
	UNamiCameraEffectComponent();
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Camera Effect Component")
	 ...
	 * 获取当前混合权重
	 */
	UFUNCTION(BlueprintPure, Category = "Camera Effect Component")
...
protected:
	 ...
	 * @param DeltaTime 帧时间
	 */
	UFUNCTION(BlueprintNativeEvent, Category = "Camera Effect Component")
	void ApplyEffect(UPARAM(ref) FNamiCameraView& InOutView, float Weight, float DeltaTime);

	/**
	 * 检查是否应该继续运行（子类可以重写）
	 * 用于检查是否有激活的震动等需要保持组件激活的情况
	 */
